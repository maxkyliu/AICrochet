...
Hunyuan3D meshes have no absolute scale, so raw measured diameters (~0.5–2.0 mesh units) would all quantize to the grammar's minimum stitch count. The measurement coordinator SHALL collect raw measurements for all measurable parts first, then apply a single global scale factor `hardcoded_max / measured_max` to every measured array before recompilation through the grammar. This preserves the mesh's relative proportions while landing values in the grammar's cm-like range.

#### Scenario: Tiny mesh diameters are scaled into grammar range
- **WHEN** raw measured diameters across all parts have a maximum value much smaller than the hardcoded diameters' maximum
- **THEN** all measured arrays are scaled by `hardcoded_max / measured_max` before recompilation, so the resulting stitch counts cover a realistic range and don't collapse to the minimum

### Requirement: Per-part measurement failure falls back to the hardcoded part
The backend SHALL recompile each part's instructions from the calibrated measured diameters via `CrochetGrammar.compile_part`, passing through the existing `primitive_type`. If measurement returns an empty array, or fails a sanity check (zero/negative values, length far from expected, wildly non-monotonic), the original hardcoded-geometry part SHALL be retained for that part.
